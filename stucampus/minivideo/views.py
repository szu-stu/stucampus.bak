...
class SignUpView(View):
    def get(self, request):
        ...
        if resource_id is None:
            form = SignUpForm()
            flag = False
            return render(request, 'minivideo/signup.html',
                          {'form': form, 'flag': flag})
        ...
        if not request.user.has_perm('minivideo.manager'):
            if 'stuno' not in request.session:
        ...
        return render(request, 'minivideo/signup.html',
                      {'form': form, 'flag': flag,
                       'resource': resource,
                       'number': number, 'personal_perm': perm})
    ...
    def post(self, request):
        ...
        resource = Resource.objects.get(
            team_captain_stuno=request.POST['team_captain_stuno'])
        ...
        form.save()
        redirect_url = '%s?id=%s' % (reverse('minivideo:details'),
                                     str(resource.id))
...
def details(request):
    ...
    if number:
        number = number.group(1)
    return render(request, 'minivideo/details.html',
...
def votes(request):
    ...
    if ticket:
        request_url = ('%sserviceValidate?ticket=%s&service=%s?id=%s'
                       % (CASserver, ticket, ReturnURL, resource_id))
        ...
        request.session['stu_no'] = tree.xpath(
            xp1, namespaces={'cas': 'http://www.yale.edu/tp/cas'})[0].text
        request.session['stu_ic'] = tree.xpath(
            xp2, namespaces={'cas': 'http://www.yale.edu/tp/cas'})[0].text
        request.session.set_expiry(0)
        ...
        if 'stu_no' in request.session:
            if len(Vote.objects.filter(stu_no=request.session['stu_no'])) < 2:
                if not Vote.objects.filter(stu_no=request.session['stu_no'],
                                           voted_id=resource.id):
                    v = Vote(stu_no=request.session['stu_no'],
                             stu_ic=request.session['stu_ic'],
                             voted_id=resource_id)
                    v.save()
                    resource.votes += 1
                    resource.save()
                    Msg = "投票成功，多谢您的支持！"
            ...
            redirect_url = ('%slogin?service=%s?id=%s'
                            % (CASserver,  ReturnURL, resource_id))
            return HttpResponseRedirect(redirect_url)
