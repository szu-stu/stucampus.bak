...
def index(request):
    ...
    if search is None:
        resource_list = (Resource.objects
                         .order_by('-is_top', '-published_date')
                         .all())
    else:
    ...
    return render(request, 'szuspeech/index.html',
                  {'page_list': page_list, 'search': search})


@check_perms('szuspeech.manager')
def manage_list(request):
    resources = Resource.objects.order_by('-is_top', '-published_date').all()
